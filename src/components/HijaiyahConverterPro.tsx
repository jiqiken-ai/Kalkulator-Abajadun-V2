...
const DATABASE_ARAB: Record<string, string> = {
  ...
  "hakim": "حكيم", "hadi": "هادي", "hidayat": "هdaية", "ihsan": "احسان", "iqbal": "اقبال",
  "ilham": "الهام", "lutfi": "لطفي", "luthfi": "لطفي", "maulana": "مولانا", "munir": "منير",
  "syafiq": "شفيق", "syahrul": "شحرul", "syamsul": "شمس ال", "salman": "سلمان",
  "alfarisi": "الفارسي", "taufik": "توفيق", "taufiq": "توفيق", "taufiqurrahman": "توفيق الرحمن",
  "wahid": "واحد", "wahyu": "واحي", "yasin": "ياسين", "yusuf": "يوسف", "yunus": "يونس",
  ...
  "hafsah": "حفصة", "rania": "رانية", "yasmin": "ياسمين", "hanifa": "حنيفة",
...
export default function HijaiyahConverterPro() {
  ...
  const rumusIndo = (word: string) => {
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      else if (char === 'e') res += isFirst ? "اي" : "ي";
    ...
    <div className="p-6 space-y-8">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-emerald-900 flex items-center justify-center gap-2">
          ...
          Konverter Hijaiyah Pro
        </h2>
        <div className="h-1 w-16 bg-emerald-500 mx-auto mt-2 rounded-full" />
      </div>

      <div className="space-y-4">
        <label className="block text-sm font-bold text-slate-700">Konversi Otomatis:</label>
        <input
          type="text"
          value={inputNama}
          onChange={(e) => setInputNama(e.target.value)}
          placeholder="Ketik nama (Contoh: Syam Retno Riyanti)"
          className="w-full p-4 border-2 border-slate-200 rounded-2xl focus:border-emerald-600 focus:outline-none transition-all text-lg"
        />
        <AnimatePresence>
          {autoResult && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-emerald-50 border-2 border-emerald-600 p-6 rounded-2xl text-center shadow-sm">
              <div className="text-5xl font-arabic text-emerald-900 mb-4 leading-relaxed" dir="rtl">{autoResult}</div>
              <button onClick={() => copyToClipboard(autoResult)} className="flex items-center gap-2 mx-auto px-4 py-2 bg-white border border-emerald-200 rounded-lg text-xs font-bold text-emerald-700 hover:bg-emerald-100 transition-colors">
      ...
      <div className="bg-blue-50 p-6 rounded-2xl border border-blue-200 shadow-sm space-y-4">
        <label className="block text-sm font-bold text-blue-900">Cari Penulisan Hijaiyah (Live Search):</label>
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 p-4 border-2 border-blue-100 rounded-xl focus:border-blue-600 focus:outline-none transition-all"
          />
          <button onClick={cariTransliterasi} className="bg-blue-800 hover:bg-blue-900 text-white px-6 rounded-xl font-bold transition-all flex items-center gap-2 active:scale-95">
            <Search size={18} /> CARI
          </button>
        </div>
        <AnimatePresence>
          {(liveResult.text || liveResult.loading) && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-xl border border-blue-200 text-center shadow-sm">
              <div className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-widest">Hasil Penulisan:</div>
              <div className="text-4xl font-arabic text-blue-900 my-4 leading-relaxed" dir="rtl">{liveResult.text}</div>
              {liveResult.suggestions.length > 0 && (
                <div className="flex flex-wrap justify-center gap-2 mt-4">
                  {liveResult.suggestions.map((s, i) => (
                    <button key={i} onClick={() => setLiveResult(prev => ({ ...prev, text: s }))} className="px-3 py-1 bg-slate-100 hover:bg-blue-100 rounded-full text-xs text-slate-600 hover:text-blue-700 transition-colors border border-slate-200">{s}</button>
                  ))}
                </div>
              )}
              <button onClick={() => copyToClipboard(liveResult.text)} className="mt-4 flex items-center gap-2 mx-auto px-4 py-2 text-xs font-bold text-blue-600 hover:text-blue-800 transition-colors">
      ...
      <div className="bg-amber-50 p-5 rounded-2xl border-l-4 border-amber-400 text-[11px] text-slate-600 leading-relaxed shadow-sm">
        <div className="flex items-center gap-2 mb-2 text-amber-800 font-bold uppercase tracking-wider"><Info size={14} /> Ketentuan Database & Rumus:</div>
        <ul className="space-y-1">
          <li>• <b>Syam:</b> Menggunakan huruf Syin (ش) ➔ <b>شم</b></li>
          <li>• <b>Retno:</b> Akhiran 'o' menjadi Alif (ا) ➔ <b>ريتنا</b></li>
          <li>• <b>Riyanti:</b> 'i' + 'y' ditulis satu 'Ya' (ي) ➔ <b>ريanتي</b></li>
        </ul>
      </div>
    </div>
  );
